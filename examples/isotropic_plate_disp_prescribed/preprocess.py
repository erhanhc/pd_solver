...
def apply_uniaxial_disp(coord,appdisp):
    left_bound = np.where(coord[:,0]<=min(coord[:,0]))[0]
    right_bound = np.where(coord[:,0]>=max(coord[:,0]))[0]
    
    indices = np.append(left_bound,right_bound)

    df = DataFrame([],columns =['dof','appliedd'],index=indices)
                   
    df.loc[left_bound,'dof']=0
    df.loc[right_bound,'dof']=0

    df.loc[left_bound,'appliedd']=-appdisp
    df.loc[right_bound,'appliedd']=appdisp

    return df

def main(W,L,thickness,dx,E,nu,appdisp):
    ...
    df4 = apply_uniaxial_disp(coord,appdisp)
...
if __name__=="__main__":
    ...
    dx = L/100.
    appdisp = .05#mm
    main(W,L,thickness,dx,E,nu,appdisp)