...
def compute_dii_simple(coord,from_to,neighbors,volumes,b,delta,thickness,mass_vec_safety_factor=5.0,dt=1.0,):
    dii = np.ones_like(coord) * simple_mass_vec(b.max(),delta,dx,dt,thickness)*mass_vec_safety_factor
    return dii
...
def main(W,L,thickness,dx,E11,E22,G12,nu12,GIC,appdisp):
    ...
    dii = compute_dii_simple(coord,from_to,neighbors,volumes,b,delta,thickness,mass_vec_safety_factor=10.0,dt=1.0,)
